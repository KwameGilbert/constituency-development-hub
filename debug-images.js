...
async function debugImages() {
  try {
    ...
    if (eventsRes.success && eventsRes.data.events.length > 0) {
      ...
      console.log("Event Image Example:", eventWithImage ? eventWithImage.image : "No event has an image");
       if (!eventWithImage && eventsRes.data.events.length > 0) {
           console.log("First Event Raw Image field:", eventsRes.data.events[0].image);
       }
    } else {
