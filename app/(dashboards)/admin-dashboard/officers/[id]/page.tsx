                ...
                <CardContent className="p-8">
                        ...
                        <div className="flex-1">
                            <div className="flex items-start gap-4">
                                ...
                                <div className="space-y-1">
                                    <h2 className="text-2xl font-bold text-gray-900">{officer.user?.name}</h2>
                                    <p className="text-gray-500">{officer.title || "Officer"} &middot; {officer.department || "General"}</p>
                                    <div className="flex gap-2 pt-1">
                                        <Badge variant="secondary" className={`${officer.user?.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                            {officer.user?.status || "Unknown"}
                                        </Badge>
                                        <Badge variant="outline" className="text-gray-500 border-gray-200">{officer.employee_id}</Badge>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-8 pt-8 border-t border-gray-100">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-3">
                                <div className="flex items-center gap-3 text-sm">
                                    ...
                                    <span className="text-gray-700">{officer.user?.email}</span>
                                </div>
                                <div className="flex items-center gap-3 text-sm">
                                    <Phone className="w-4 h-4 text-gray-400" />
                                    <span className={`${officer.user?.phone ? "text-gray-700" : "text-gray-400 italic"}`}>
                                        {officer.user?.phone || "No phone number"}
                                    </span>
                                </div>
                                <div className="flex items-center gap-3 text-sm">
                                    <Calendar className="w-4 h-4 text-gray-400" />
                                    <span className="text-gray-700">Joined {new Date(officer.created_at).toLocaleDateString()}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                                ...
                                {officer.assigned_locations && officer.assigned_locations.length > 0 ? (
                                    <div className="flex flex-wrap gap-2">
                                        {officer.assigned_locations.map((loc, idx) => (
                                            <Badge key={idx} variant="outline" className="bg-gray-50">{loc}</Badge>
                                        ))}
                                    </div>
                                ) : (
                            ...
                            {officer.assigned_sectors && officer.assigned_sectors.length > 0 && (
                                <>
                                 <h3 className="font-semibold text-gray-900 mt-4">Assigned Sectors</h3>
                                 <div className="flex flex-wrap gap-2">
                                    {officer.assigned_sectors.map((sec, idx) => (
                                         <Badge key={idx} variant="secondary" className="bg-indigo-50 text-indigo-700">{sec}</Badge>
                                    ))}
                                 </div>
                                </>
                            )}
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="text-center p-3 bg-indigo-50/50 rounded-lg">
                                    <div className="text-2xl font-bold text-indigo-600">{officer.pending_reports_count || 0}</div>
                            ...
                            <div className="pt-4 border-t border-gray-100 flex items-center justify-center gap-2">
                                <div className="flex flex-col items-center">
                                    <span className="text-xl font-bold text-purple-600">{officer.supervised_agents_count || 0}</span>
