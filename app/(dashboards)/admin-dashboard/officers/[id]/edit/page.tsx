...
import { Loader2, User, Eye, UserCircle, ShieldAlert, Settings2, LogOut } from "lucide-react";
import Link from "next/link";
import { useState, useEffect, use } from "react";
import { useRouter } from "next/navigation";
import { officersService, UpdateOfficerRequest, Officer } from "@/lib/services/officers-service";
...
export default function EditOfficerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSelectChange = (key: keyof UpdateOfficerRequest, value: string) => {
  ...
  const handleCheckboxChange = (key: keyof UpdateOfficerRequest, checked: boolean) => {
            ...
            <Card className="border-t-4 border-t-indigo-900">
                <CardHeader>
                    <div className="flex items-center gap-2">
                        <CardTitle className="text-xl text-gray-900">Edit Officer: {officer?.user?.name}</CardTitle>
                    </div>
                    <CardDescription>Update agent details and permissions</CardDescription>
                </CardHeader>
                <CardContent className="space-y-8 pt-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-2">
                                ...
                                <Input id="email" type="email" value={officer?.user?.email} disabled className="bg-gray-100 cursor-not-allowed" />
                    ...
                    <div className="pt-4 flex items-center justify-end gap-3 border-t border-gray-100 mt-6">
                         ...
                         <Button type="submit" className="bg-indigo-600 hover:bg-indigo-700 text-white gap-2" disabled={submitting}>
                            {submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : "Update Officer"}
                         </Button>
                    </div>
