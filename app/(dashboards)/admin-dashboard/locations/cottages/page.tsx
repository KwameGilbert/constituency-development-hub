...
export default function CottagesPage() {
    ...
    const fetchCottages = useCallback(async () => {
        try {
            ...
            if (response.success && response.data?.locations) {
                const mappedCottages: Cottage[] = response.data.locations.map((loc: Location) => ({
                    id: loc.id,
                    name: loc.name,
                    parent_id: loc.parent_id,
    ...
    const handleAddCottage = async () => {
        ...
        const parentId = (selectedSmallerCommunity && selectedSmallerCommunity !== "none") 
    ...
    const handleSaveChanges = async () => {
        ...
        const parentId = (editSmallerCommunityId && editSmallerCommunityId !== "none")
            ? parseInt(editSmallerCommunityId)
            : parseInt(editSuburbId);

        setIsSubmitting(true);
        try {
            const response = await locationsService.updateLocation(editingCottage.id, {
                    ...
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm space-y-6">
                        <div className="space-y-1">
                            ...
                            <p className="text-sm text-gray-500">
                                {loading ? "Loading..." : `Showing ${filteredCottages.length} cottages`}
                            </p>
                        </div>
                                    ...
                                    {filteredCottages.map((cottage) => {
                                        ...
                                        return (
                                            <TableRow key={cottage.id} className="hover:bg-gray-50/50 border-gray-100 transition-colors">
                                                ...
                                                <TableCell className="text-gray-500">{suburbName}</TableCell>
                                                <TableCell className="text-gray-500">{smallerCommunityName}</TableCell>
                ...
                <DialogContent className="sm:max-w-[500px]">
                    ...
                    <div className="py-4 space-y-4">
                        <div className="space-y-2">
                            <Label>Suburb</Label>
                            <Select value={selectedSuburb} onValueChange={(val) => {
                        ...
                        <div className="space-y-2">
                            ...
                            <Select value={selectedSmallerCommunity} onValueChange={setSelectedSmallerCommunity} disabled={isSubmitting || !selectedSuburb}>
                                <SelectTrigger className="w-full">
            ...
            <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
                <DialogContent className="sm:max-w-[500px]">
                    ...
                    <div className="py-4 space-y-4">
                        <div className="space-y-2">
                            <Label>Suburb</Label>
                            <Select value={editSuburbId} onValueChange={(val) => {
                                setEditSuburbId(val);
                                setEditSmallerCommunityId("none");
                            }} disabled={isSubmitting}>
                                <SelectTrigger className="w-full">
                        ...
                        <div className="space-y-2">
                            <Label>Smaller Community (Optional)</Label>
                            <Select value={editSmallerCommunityId} onValueChange={setEditSmallerCommunityId} disabled={isSubmitting || !editSuburbId}>
                                <SelectTrigger className="w-full">
                                    <SelectValue placeholder="None" />
                                </SelectTrigger>
                                <SelectContent>
