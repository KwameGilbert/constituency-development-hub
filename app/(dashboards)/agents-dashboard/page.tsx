...
function AgentMainPage() {
  ...
  const breakdownChartData = useMemo(() => {
    ...
    const categoryData: BreakdownChartDataItem[] = Object.entries(catCounts)
      .map(([cat, count]) => {
    ...
    if (categoryData.length === 0)
      categoryData.push({ label: "No Data", count: 0, fill: "#6b7280" });
    ...
    const priorityData: BreakdownChartDataItem[] = Object.entries(priCounts)
      .map(([pri, count]) => ({
      ...
      .sort((a, b) => {
        return (
          priorityOrder.indexOf(a.label.toLowerCase()) -
          priorityOrder.indexOf(b.label.toLowerCase())
        );
      });

    if (priorityData.length === 0)
      priorityData.push({ label: "No Data", count: 0, fill: "#6b7280" });
