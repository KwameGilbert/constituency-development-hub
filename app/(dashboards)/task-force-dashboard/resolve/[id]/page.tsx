...
export default function ResolveIssuePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.resolution_summary || !formData.work_description || !formData.completion_date) {
    ...
    try {
      ...
      if (formData.actual_cost) submitData.append("actual_cost", formData.actual_cost);
      if (formData.challenges_faced) submitData.append("challenges_faced", formData.challenges_faced);
      ...
      const response = await taskForceService.submitResolution(parseInt(id), submitData);
      ...
      <main className="flex-1 p-4 md:p-8 max-w-4xl mx-auto w-full">
        <div className="mb-6">
          <Link href={`/task-force-dashboard/issues/${id}`}>
             <Button variant="ghost" size="sm" className="pl-0 hover:bg-transparent text-slate-500 hover:text-slate-800">
               <ArrowLeft className="mr-2 h-4 w-4" />
               Back to Issue Details
             </Button>
          </Link>
          <div className="mt-2">
            <h1 className="text-2xl font-bold text-slate-900">Resolved Issue Report</h1>
            <p className="text-slate-500">Submit your findings and work completion details for Case #{issue.case_id}</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <Card>
             <CardHeader>
                ...
                <CardDescription>Describe the work done to resolve the issue</CardDescription>
             </CardHeader>
          ...
          <Card>
             ...
             <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                   ...
                   />
                   <p className="text-xs text-gray-500">Total cost incurred during resolution. Leave blank if N/A.</p>
                </div>
             </CardContent>
          </Card>
          ...
          <Card>
             <CardHeader>
                ...
                <CardDescription>Upload photos showing the work before and after completion</CardDescription>
             </CardHeader>
                ...
                <div className="space-y-3">
                   ...
                   <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {beforePreviews.map((src, idx) => (
                         <div key={idx} className="relative aspect-square rounded-md overflow-hidden border bg-gray-100 group">
                            <Image src={src} alt={`Before ${idx}`} fill className="object-cover" />
                            <button
                               type="button"
                      ...
                      <label className="flex flex-col items-center justify-center aspect-square border-2 border-dashed rounded-md cursor-pointer hover:bg-slate-50 transition-colors">
                         ...
                         <input type="file" className="hidden" multiple accept="image/*" onChange={(e) => handleImageChange(e, 'before')} />
                      </label>
                   </div>
                </div>
                ...
                <div className="space-y-3">
                   ...
                   <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {afterPreviews.map((src, idx) => (
                         <div key={idx} className="relative aspect-square rounded-md overflow-hidden border bg-gray-100 group">
                            <Image src={src} alt={`After ${idx}`} fill className="object-cover" />
                            <button
                               type="button"
                      ...
                      <label className="flex flex-col items-center justify-center aspect-square border-2 border-dashed rounded-md cursor-pointer hover:bg-slate-50 transition-colors">
                         ...
                         <input type="file" className="hidden" multiple accept="image/*" onChange={(e) => handleImageChange(e, 'after')} />
                      </label>
                   </div>
                </div>
             </CardContent>
          ...
          <div className="flex justify-end gap-3 pt-4">
             ...
             <Button type="submit" disabled={isSubmitting} className="bg-green-600 hover:bg-green-700 min-w-[150px]">
                {isSubmitting ? (
                   <>
