...
export default function ReportsPage() {
  ...
  const resolutionRate = reports?.total_issues ? Math.round((reports.resolved_issues / reports.total_issues) * 100) : 0;
  const automatedInsights = useMemo(() => {
    const insights = [];
    if (resolutionRate > 80) insights.push("✅ Excellent resolution rate above 80%.");
    else if (resolutionRate < 50) insights.push("⚠️ Resolution rate is below 50%, requiring attention.");
    
    if ((reports?.priority_distribution?.urgent || 0) > 5) {
      insights.push(`🚨 High number of Urgent issues (${reports?.priority_distribution?.urgent}).`);
    }

    if (daysDiff < 7) insights.push("ℹ️ Report covers a short period (< 7 days).");
  ...
  const handleDetailedExport = () => {
    ...
    const summaryData = [
      ...
      ["Report Period", `${date?.from ? format(date.from, "PPP") : "N/A"} - ${date?.to ? format(date.to, "PPP") : "N/A"}`],
    ...
    // -- SHEET 2: DATA --
    const statusRows = statusData.map(s => ({ Status: s.name, Count: s.value }));
    ...
    XLSX.writeFile(workbook, `TaskForce_Report_${format(new Date(), "yyyy-MM-dd")}.xlsx`);
  };
  ...
  return (
    <div className="p-8 max-w-[1600px] mx-auto space-y-8 bg-gray-50/50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b pb-6 print:hidden">
        <div>
          ...
          <p className="text-gray-500 mt-2 text-lg">
            Comprehensive reporting with interpretation and specific date filtering.
          </p>
        </div>
      ...
      <div id="report-content" ref={reportRef} className="space-y-8 bg-white p-8 rounded-xl shadow-sm border border-gray-100">
        
        {/* Report Header in PDF */}
        <div className="flex justify-between items-center border-b pb-4 mb-4">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">Task Force Report</h2>
            <p className="text-gray-500 text-sm">
              Generated: {format(new Date(), "PPP")} | Period: {date?.from ? format(date.from, "PPP") : "..."} - {date?.to ? format(date.to, "PPP") : "..."}
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <CardContent className="p-4">
                <p className="text-sm font-medium text-gray-500 uppercase">Total Issues</p>
                <h3 className="text-3xl font-bold text-gray-900 mt-2">{reports?.total_issues || 0}</h3>
            </CardContent>
            </Card>
            ...
            <CardContent className="p-4">
                ...
                <h3 className="text-3xl font-bold text-gray-900 mt-2">{reports?.resolved_issues || 0}</h3>
            </CardContent>
            </Card>
            <Card className="bg-gray-50 border-l-4 border-l-blue-500">
            <CardContent className="p-4">
                <p className="text-sm font-medium text-gray-500 uppercase">Pending</p>
                <h3 className="text-3xl font-bold text-gray-900 mt-2">{reports?.status_distribution?.assigned_to_task_force || 0}</h3>
            </CardContent>
            </Card>
            <Card className="bg-gray-50 border-l-4 border-l-orange-500">
            <CardContent className="p-4">
                <p className="text-sm font-medium text-gray-500 uppercase">Efficiency</p>
                <h3 className="text-3xl font-bold text-gray-900 mt-2">{resolutionRate}%</h3>
            </CardContent>
            </Card>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 print:gap-4 print:grid-cols-2">
            ...
            <div className="border rounded-lg p-6 break-inside-avoid">
                ...
                <div className="mt-4 bg-blue-50 p-3 rounded text-sm text-blue-900">
                    <strong>Interpretation:</strong> {statusData[0]?.value > 10 ? "Backlog needs attention." : "Workload is manageable."}
                </div>
            </div>
        </div>
              ...
              <TableBody>
                {teamMembers.map((member) => (
                  <TableRow key={member.id}>
                    ...
                    <TableCell className="text-muted-foreground text-xs">{member.title || "Officer"}</TableCell>
                    <TableCell className="text-center">{member.assessments_completed}</TableCell>
                    <TableCell className="text-center">{member.resolutions_completed}</TableCell>
                    <TableCell className="text-center">
                       <Badge variant={member.completion_rate > 80 ? "default" : "secondary"}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 print:gap-4 print:grid-cols-2 break-inside-avoid">
            ...
            <div className="border rounded-lg p-6 bg-yellow-50/30 border-yellow-100">
                <h3 className="text-lg font-bold mb-2 text-yellow-900">Officer Notes & Interpretation</h3>
                <p className="text-xs text-yellow-700 mb-2 print:hidden">Add your analysis before exporting to PDF.</p>
                <Textarea 
