...
import { Lightbulb, ThumbsUp, ThumbsDown, CheckCircle, Clock, AlertCircle } from 'lucide-react';
...
export default function IdeasClient() {
  ...
  const handleVote = async (id: number, voteType: 'up' | 'down') => {
    try {
      setVotingId(id);
      const response = await ideasService.voteIdea(id, voteType);
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'implemented': return 'bg-green-500 hover:bg-green-600';
      case 'approved': return 'bg-blue-500 hover:bg-blue-600';
      case 'under_review': return 'bg-yellow-500 hover:bg-yellow-600';
    ...
    return (
      <div className="container mx-auto px-4 py-8 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} className="h-64 w-full bg-gray-100 rounded-lg animate-pulse" />
          ))}
        </div>
      </div>
    );
      ...
      <div className="py-12">
        ...
        {error ? (
          <div className="bg-red-50 border border-red-200 rounded-lg p-6 flex flex-col items-center justify-center text-center">
            ...
            <Button onClick={fetchIdeas} variant="outline" className="border-red-200 text-red-700 hover:bg-red-50">
              Try Again
            </Button>
          </div>
        ) : ideas.length === 0 ? (
          <div className="text-center py-12 bg-white rounded-lg border shadow-sm">
            ...
            <p className="text-gray-500">There are no approved community ideas to display at the moment.</p>
            <div className="mt-6">
               <SubmitIdeaDialog trigger={
            ...
            {ideas.map((idea, index) => (
              ...
              >
                <Card className="h-full hover:shadow-lg transition-all duration-300 flex flex-col overflow-hidden group">
                  <div className={`h-2 w-full ${idea.status === 'implemented' ? 'bg-green-500' : 'bg-blue-500'}`} />
                  ...
                  <CardContent className="p-6 flex-1 flex flex-col">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <Badge className={`${getStatusColor(idea.status)} text-white border-0 capitalize`}>
                    ...
                    <div className="mt-auto space-y-4">
                      <div className="flex items-center justify-between text-sm text-gray-500 pt-4 border-t border-gray-100">
                        <div className="flex items-center gap-3">
                          ...
                          >
                            <ThumbsUp className={`h-4 w-4 ${idea.votes && idea.votes > 0 ? 'text-blue-500 fill-blue-50' : ''} group-hover/vote:scale-110 transition-transform`} />
                          ...
                          >
                            <ThumbsDown className={`h-4 w-4 ${idea.downvotes && idea.downvotes > 0 ? 'text-red-500 fill-red-50' : ''} group-hover/downvote:scale-110 transition-transform`} />
                      ...
                      {idea.status === 'implemented' && (
                        <div className="pt-2">
                          <div className="flex justify-between text-xs mb-1 font-medium text-green-700">
                            <span className="flex items-center gap-1"><CheckCircle className="h-3 w-3" /> Implemented</span>
                            <span>100%</span>
                          </div>
