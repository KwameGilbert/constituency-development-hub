...
import { ReportData, ReportColumn, ReportRow } from "@/lib/services/reports-service";
...
export function ReportPreview({ data, columns }: ReportPreviewProps) {
  ...
  const formatCellValue = (columnId: string, value: string | number | null): React.ReactNode => {
    ...
    if (columnId === 'status') {
      ...
      return <Badge variant="outline" className={colorClass}>{String(value)}</Badge>;
    }
      ...
      <CardHeader className="bg-gray-50/50 pb-4 border-b border-gray-100">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">
            {data.reportType.charAt(0).toUpperCase() + data.reportType.slice(1)} Report
          </Badge>
        </div>
      </CardHeader>
