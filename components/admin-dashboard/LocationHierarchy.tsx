...
export function LocationHierarchy() {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-8">
      <h3 className="text-lg font-bold text-gray-900 mb-12">Location Hierarchy</h3>
        ...
        <div className="space-y-12">
          {steps.map((step) => (
            <div key={step.id} className="relative flex items-center justify-between">
              ...
              {/* Left Side */}
              <div className={`flex-1 flex ${step.align === 'left' ? 'justify-end pr-12 text-right' : 'justify-start pl-12'}`}>
                 {step.align === 'left' ? (
                    // Text Content on Left
                    <div className="space-y-2">
                        <h4 className={`text-xl font-bold ${step.textColor}`}>{step.title}</h4>
                        <p className="text-gray-500 max-w-xs ml-auto">{step.description}</p>
                        <Link 
                            href={step.href}
                    ...
                    <div className="flex items-center gap-2 text-gray-600">
                        <CheckCircle2 className={`w-5 h-5 ${step.textColor.replace('text-', 'text-emerald-500') /* Status icon is green in mock usually, but let's stick to theme or default green success if implies valid */ } text-green-500`} /> 
                        <span className="font-medium text-gray-600">{step.count} {step.countLabel}</span>
                    </div>
                 )}
              </div>
              ...
              {/* Right Side */}
              <div className={`flex-1 flex ${step.align === 'left' ? 'justify-start pl-12' : 'justify-end pr-12 text-right'}`}>
                 {step.align === 'left' ? (
