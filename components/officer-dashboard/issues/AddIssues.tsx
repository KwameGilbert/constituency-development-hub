...
export function AddIssues() {
        ...
        const loadLocations = async () => {
            try {
                const [coms, subs, smalls] = await Promise.all([
                    ...
                    locationsService.getLocations({ type: 'smaller_community', limit: 100 })
                ]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const getLocName = (id: string, list: Location[]) => list.find(l => l.id.toString() === id)?.name || id;
    ...
    if (loading) {
        return <div className="p-8 flex justify-center"><Loader2 className="animate-spin h-8 w-8 text-indigo-600" /></div>;
    }
                ...
                <TabsContent value="issue-details" className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <FormItem label="Subsector">
                            <Select value={formData.subsector} onValueChange={(v) => handleChange("subsector", v)}>
                                <SelectTrigger>
                ...
                <TabsContent value="constituent-details" className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <FormItem label="Gender">
                            <Select value={formData.constituent_gender} onValueChange={(v) => handleChange("constituent_gender", v)}>
                                <SelectTrigger>
                ...
                <TabsContent value="location" className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <FormItem label="Main Community" required>
                            <Select value={formData.location_community} onValueChange={(v) => handleChange("location_community", v)}>
                                <SelectTrigger>
                        ...
                        <FormItem label="Smaller Community">
                            <Select value={formData.location_smaller_community} onValueChange={(v) => handleChange("location_smaller_community", v)}>
                                <SelectTrigger>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <FormItem label="Suburb">
                            <Select value={formData.location_suburb} onValueChange={(v) => handleChange("location_suburb", v)}>
                                <SelectTrigger>
                    ...
                    <div className="flex justify-between items-center pt-4">
                        <p className="text-sm text-red-500">* Required fields</p>
                        <div className="flex gap-2">
                            <Button variant="secondary" onClick={() => handleNext("constituent-details")} disabled={submitting}>
                                <ArrowLeft className="mr-2 h-4 w-4" /> Previous
                            </Button>
                            <Button 
