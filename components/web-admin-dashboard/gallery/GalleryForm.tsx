...
const gallerySchema = z.object({
  ...
  status: z.string(),
  gallery_items: z.array(z.object({
    file: z.any().optional(),
...
export default function GalleryForm({ gallery }: GalleryFormProps) {
  ...
  const form = useForm<GalleryFormValues>({
    ...
    defaultValues: {
      ...
      gallery_items: gallery?.images?.map(img => ({ url: img.url, caption: img.caption })) || [],
    },
  });
  ...
  const onSubmit = async (data: GalleryFormValues) => {
    try {
      ...
      if (gallery) {
        ...
        const response = await galleryService.updateGallery(gallery.id, updatePayload);
        if (response.success) {
              ...
              <div className="space-y-4">
                <div 
                  className={`relative aspect-[4/3] rounded-xl border-2 border-dashed overflow-hidden flex items-center justify-center transition-all ${
                    coverPreview ? "border-solid border-emerald-100" : "border-slate-200 hover:border-emerald-300 bg-slate-50"
                  }`}
                >
                  {coverPreview ? (
                    <>
                    ...
                    <div className="text-center p-6">
                      ...
                      <p className="text-xs font-medium text-slate-600">Click to upload cover</p>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="md:col-span-2">
                      ...
                      <FormControl>
                        <Input placeholder="Enter album title..." {...field} className="bg-slate-50 border-slate-200 h-11" />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input type="date" {...field} className="bg-slate-50 border-slate-200 h-11" />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="e.g. Sefwi Wiawso" {...field} className="bg-slate-50 border-slate-200 h-11" />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
               ...
               <div className="space-y-4">
                 {fields.length === 0 ? (
                   <div className="text-center py-12 border-2 border-dashed border-slate-100 rounded-xl bg-slate-50/50">
                      ...
                      <p className="text-sm text-slate-400">No gallery photos added yet</p>
                      <Button 
                        type="button" 
                        variant="link" 
                   ...
                   <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                     {fields.map((field, index) => (
                       <div key={field.id} className="relative group bg-slate-50 rounded-xl border border-slate-100 overflow-hidden p-3 pt-4">
                          <button
                            type="button"
                          ...
                          <div className="space-y-3">
                             ...
                             <div className="aspect-video bg-white rounded-lg border border-slate-200 overflow-hidden relative flex items-center justify-center group-hover:border-emerald-200 transition-colors">
                                {form.watch(`gallery_items.${index}.file`) ? (
                                  ...
                                  /* eslint-disable-next-line @next/next/no-img-element */
                                  <img 
                                    src={getImageUrl(form.watch(`gallery_items.${index}.url`)!)} 
                                    alt="Existing" 
                                    className="w-full h-full object-cover"
                                  />
                                ) : (
                                  <div className="text-center">
                                     ...
                                     <span className="text-[10px] font-medium text-slate-400">Upload Image</span>
                                  </div>
                                )}
                                <input
                                  type="file"
            ...
            <div className="flex items-center justify-end gap-4 pt-4">
               ...
               >
                 {loading ? (
                   ...
                   gallery ? "Update Album" : "Create Album"
                 )}
               </Button>
            </div>
          </div>
